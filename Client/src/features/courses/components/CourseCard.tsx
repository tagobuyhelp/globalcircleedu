...
export const CourseCard: React.FC<CourseCardProps> = ({ 
  ...
  return (
    <Card className={`overflow-hidden ${view === 'list' ? 'flex' : 'h-full'}`}>
      <div className={`flex ${view === 'list' ? 'flex-row' : 'flex-col'}`}>
        <div className={view === 'list' ? 'w-64 flex-shrink-0' : 'w-full'}>
          <img
            src={course.program.university.image || 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1'}
            alt={course.name}
            className={`object-cover ${view === 'list' ? 'h-full' : 'h-48 w-full'}`}
          />
        </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-4">
                <div className="flex items-center text-sm">
                  ...
                  <span>{course.program.degree.name}</span>
                </div>
                <div className="flex items-center text-sm">
