...
export const createVisitor = asyncHandler(async (req, res) => {
    ...
    res
        .status(201)
        .json(new ApiResponse(201, visitor, "Visitor created successfully"));
});
...
export const getVisitorById = asyncHandler(async (req, res) => {
    ...
    res
        .status(200)
        .json(new ApiResponse(200, visitor, "Visitor fetched successfully"));
});
...
for (const field of documentFields) {
    ...
    if (req.files && req.files[fileField] && req.files[fileField].length > 0) {
        console.log(`Processing files for ${field}`);

        // Ensure documents object exists for the given field
        if (!updateData.documents[field]) {
            updateData.documents[field] = {};
        }

        const file = req.files[fileField][0]; // Accessing the first file correctly
        ...
        const version = versionMatch ? versionMatch[1] : 'v1'; // Default to 'v1' if version is missing
        ...
        updateData.documents[field] = {
            name: file.originalname,
            fileURL: optimizedUrl, // Use the optimized URL
            documentType: file.mimetype,
        };
    } else {
        console.log(`No files uploaded for ${field}`);
    }
}
    ...
    const nestedFields = ["address", "education", "professional"];
    for (const field of nestedFields) {
        if (updateData[field] && typeof updateData[field] === "object") {
            updateData[field] = { ...visitor[field], ...updateData[field] };
        }
    }

    // Handle array fields
    const arrayFields = ["interestedCourse", "interestedJob"];
    for (const field of arrayFields) {
        if (updateData[field]) {
            if (Array.isArray(updateData[field])) {
                // If it's an array, replace the entire array
                // Ensure all elements are valid ObjectIds
                updateData[field] = updateData[field].filter((id) =>
                    mongoose.Types.ObjectId.isValid(id)
                );
            } else if (mongoose.Types.ObjectId.isValid(updateData[field])) {
                // If it's a single valid ObjectId, convert it to an array
                updateData[field] = [updateData[field]];
            } else {
                // If it's neither a valid array nor a valid ObjectId, remove it from updateData
    ...
    res
        .status(200)
        .json(new ApiResponse(200, visitor, "Visitor updated successfully"));
});
...
export const deleteVisitor = asyncHandler(async (req, res) => {
    ...
    res
        .status(200)
        .json(new ApiResponse(200, null, "Visitor deleted successfully"));
});
...
export const bulkCreateVisitors = asyncHandler(async (req, res) => {
    ...
    res
        .status(201)
        .json(new ApiResponse(201, visitors, "Visitors created successfully"));
});
...
export const getVisitorStats = asyncHandler(async (req, res) => {
    const [visitorStats, universitiesCount, coursesCount, jobsCount, newsCount] =
        await Promise.all([
            Visitor.aggregate([
                {
                    $facet: {
                        basicStats: [
                            {
                                $group: {
                                    ...
                                    studentCount: {
                                        $sum: {
                                            $cond: [{ $eq: ["$visitorType", "Student"] }, 1, 0],
                                        },
                                    },
                                    workerCount: {
                                        $sum: {
                                            $cond: [{ $eq: ["$visitorType", "Worker"] }, 1, 0],
                                        },
                                    },
                        ...
                        consultationBookingRate: [
                            {
                                $group: {
                                    ...
                                    totalVisitors: { $sum: 1 },
                                    bookedConsultations: {
                                        $sum: { $cond: ["$isConsultationBooked", 1, 0] },
                                    },
                                },
                            },
                            {
                                $project: {
                                    bookingRate: {
                                        $divide: ["$bookedConsultations", "$totalVisitors"],
                                    },
                                },
                            },
                        ],
                ...
                },
                {
                    $project: {
                        ...
                        consultationBookingRate: {
                            $arrayElemAt: ["$consultationBookingRate", 0],
                        },
                    },
                },
            ]),
    ...
    result.basicStats.averageAge = result.basicStats.averageAge
        ...
        : null;
    result.basicStats.averageEducationLevel = result.basicStats
        .averageEducationLevel
        ? parseFloat(result.basicStats.averageEducationLevel.toFixed(1))
        : null;

    // Calculate total unique countries
    result.totalUniqueCountries = result.topCountries
        ? result.topCountries.length
        : 0;
    ...
    res
        ...
        .json(
            new ApiResponse(
                200,
                result,
                "Advanced visitor statistics fetched successfully"
            )
        );
});
